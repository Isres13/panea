            ...
            <div class=" col-lg-10 mx-auto">
                <div class="row mt-5">
                    <div class="col-md-12 ">
                        ...
                        <a href="{{route('DWmoney.index')}}" class="btn btn-primary my-3 float-start ">ย้อนกลับ</a>
                    </div>
                </div>
                <div class="row">
                    <div class="col-8">
                        <p class="h5 alert alert-light text-dark" >ชื่อ: {{$liats[0]->name}} {{$liats[0]->lname}}</p>
                    </div>
                    <div class="col">
                        <p class=" text-center h5  alert alert-secondary text-dark">ยอดเงิน : {{$liats[0]->balance}}</p>
                    </div>
                </div>
                
                @foreach( $liats as $row )
                    @if($row->type == 0)
